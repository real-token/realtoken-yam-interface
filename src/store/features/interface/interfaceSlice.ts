...
export const propertiesChanged = createAction<APIPropertiesToken[]>(propertiesChangedDispatchType);
export const propertiesIsLoading = createAction<boolean>(propertiesIsLoadingDispatchType);
export const chainPropertiesChanged = createAction<PropertiesToken[]>(chainPropertiesChangedDispatchType)
  ...
  return async function fetchPropertiesThunk(dispatch: any,) {
    try{
      ...
      if(response.ok){
        ...
        dispatch({ type: propertiesChangedDispatchType, payload: responseJson })
...
export const interfaceReducers = createReducer(interfaceInitialState, (builder) => {
  builder.addCase(offersChanged, (state, action) => {
