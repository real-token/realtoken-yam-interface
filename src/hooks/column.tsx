      ...
      >
        <Flex>
          ...
          <ActionIcon
            variant={'transparent'}
            color={'blue'}
            onClick={() => handleCopyClick(getValue())}
          >
            <IconCopy size={18} />
          </ActionIcon>
        </Flex>
...
export const amountColumn: ColumnFn<string> = (t, span) => {
  return {
    ...
    filterFn: (row, columnId, filterValue: [string | null, string | null]) => {
      const [min, max] = filterValue;
      const amount = row.getValue(columnId) as string;
      if (min === null && max === null) return true;
      if (min === null && max !== null)
        return BigNumber(amount).isLessThanOrEqualTo(max);
      if (max === null && min !== null)
        return BigNumber(amount).isGreaterThanOrEqualTo(min);
      if (max && min) {
        return (
          BigNumber(amount).isGreaterThanOrEqualTo(min) &&
          BigNumber(amount).isLessThanOrEqualTo(max)
        );
      }
      return true;
    },
...
export const adminAmount: ColumnFn<string> = (t, span) => {
  return {
    ...
    cell: ({ getValue }) => (
      ...
      >
        {getValue() ? <Text>{getValue()}</Text> : <Skeleton height={15} />}
      </Text>
    ),
    enableSorting: true,
    meta: { colSpan: span },
