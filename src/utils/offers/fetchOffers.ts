  ...
  return new Promise<Offer>(async (resolve,reject) => {
    try{
      ...
      const o: Offer = {
        ...
        amount: BigNumber.minimum(offer.availableAmount, balanceWallet, allowance).toString(10) ?? '0',
...
export const fetchOfferTheGraph = (chainId: number, propertiesToken: PropertiesToken[]) : Promise<Offer[]> => {
  return new Promise<Offer[]>(async (resolve, reject) => {
    try{
      ...
      await Promise.all(
        data.offers.map(async (offer: OfferGraphQl) => {
          ...
          const hasPropertyToken = propertiesToken.find(propertyToken => (propertyToken.contractAddress == offerData.buyerTokenAddress || propertyToken.contractAddress == offerData.offerTokenAddress));
