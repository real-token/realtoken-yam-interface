...
export const MultiPathDetailsPopover = ({
  averagePrice,
}: MultiPathDetailsPopoverProps) => {
  const { t } = useTranslation('modals', { keyPrefix: 'offerMatching' });
  const [opened, { open, close }] = useDisclosure(false);

  return (
    <Popover width={'target'} position={'top'} withArrow={true} opened={opened}>
      <Popover.Target>
        <Flex gap={6} onMouseEnter={open} onMouseLeave={close} align={'center'}>
      ...
      <Popover.Dropdown>
        ...
        <ul>
          {Object.keys(averagePrice.details).map((key, index) => {
            ...
            return (
              <li key={`detail-${index}`}>{`$ ${detailValue} of ${key}`}</li>
            );
          })}
        </ul>
      </Popover.Dropdown>
