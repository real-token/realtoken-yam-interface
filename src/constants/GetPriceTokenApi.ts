...
// Simplified token data for API routes (no React/SVG imports)
export interface ApiPriceToken {
  name: string;
  symbol: string;
  contractAddress: string;
  priceFnc:
    | { type: 'chainlink'; contractAddress: string }
    | { type: 'coingecko-api'; address?: string }
    | { type: 'custom-fnc'; fnc: () => Promise<number> };
}

export const tokenToGetPriceApi = new Map<number, ApiPriceToken[]>([
  [
    ...
    [
      {
        ...
        symbol: 'REUSD',
        contractAddress: '0x3390742Ac0DCe14EA6Fcbd5Ae02e2303C5D62Ad9',
        priceFnc: {
          type: 'custom-fnc',
